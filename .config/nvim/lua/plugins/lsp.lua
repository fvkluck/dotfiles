...
local function _1_()
  ...
  vim.lsp.config("ts-ls", {on_attach = on_attach, cmd = {"typescript-language-server", "--stdio"}, init_options = {plugins = {{name = "@vue/typescript-plugin", location = "/opt/homebrew/lib/node_modules/@vue/typescript-plugin", languages = {"javascript", "typescript", "vue"}}}}, filetypes = {"javascript", "typescript", "vue"}, handlers = handlers, before_init = before_init, capabilities = capabilities})
  vim.lsp.config("pyright", {on_attach = on_attach, handlers = handlers, before_init = before_init, capabilities = capabilities})
  vim.lsp.config("rust-analyzer", {on_attach = on_attach, handlers = handlers, before_init = before_init, capabilities = capabilities})
